...
def _base_rows():
    ...
    return [{default_pk: i, default_vec: _vector(i), json_field: payload} for i, payload in enumerate(payloads)]


def _rows(total=20):
    rows = _base_rows()
    for i in range(len(rows), total):
        rows.append({default_pk: i, default_vec: _vector(i), json_field: {"pad": i}})
    return rows
...
@pytest.mark.xdist_group("TestJsonFilteringUnknownSemantics")
...
@pytest.mark.xdist_group("TestJsonFilteringIndexedUnknownSemantics")
